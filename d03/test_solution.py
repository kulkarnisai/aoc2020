...
coordinates in a grid. You make a map (your puzzle input) of the open squares (`.`) and trees (`#`) you can see. 
For example:
...
row on your map). The toboggan can only follow a few specific slopes (you opted for a cheaper model that prefers 
...
import sys
import numpy as np
def trees_encountered(arr, slope):
    trees = 0
    r, c = 0, 0
...
def all_slopes(arr):    
    ...
    trees = [trees_encountered(arr, slope) for slope in slopes]
...
def test_day3(outfile):
    ...
    assert trees_encountered(test_arr, [3, 1]) == 7
    print("P1:\t" + str(trees_encountered(arr, [3, 1])))

    assert all_slopes(test_arr) == 336
    print("P2:\t" + str(all_slopes(arr)) + codeblock)
